    ...
    def __call__(self, results):
        ...
        for packet in container.demux(stream):
            for frame in packet.decode():
                ...
                height = frame.height
                width = frame.width
                mv = np.zeros((height, width, 2), dtype=int8)
    ...
    def __call__(self, results):
        ...
        if results['fmt'] == 'video':
            ...
            for frame_idx in results['frame_inds']:
                ...
                if modality == 'RGB':
                    try:
                        cur_frame = video[cur_index]
                    except:
                        if cur_index == 0:
                            # cur_frame = video.get_data(cur_index+1)
                            cur_frame = video[cur_index+1]
                        else:
                            cur_frame = video[cur_index-1]
                    imgs.append(cur_frame)
                    ...
                    try:
                        cur_frame_x = video_x[cur_index]
                    except:
                        if cur_index == 0:
                            cur_frame_x = video_x[cur_index+1]
                        else:
                            cur_frame_x = video_x[cur_index-1]
                    # try:
                    ...
                    try:
                        cur_frame_y = video_y[cur_index]
                    except:
                        if cur_index == 0:
                            cur_frame_y = video_y[cur_index+1]
                        else:
                            try:
                                cur_frame_y = video_y[cur_index-1]
                            except:
                                try:
                                    cur_frame_y = video_y[cur_index-2]
                                except:
                                    try:
                                        cur_frame_y = video_y[cur_index-3]
                                    except:
                                        print("directory: ", directory)
                                        print("frame_num: ", frame_num)
                                        cur_frame_y = video_y[0]
