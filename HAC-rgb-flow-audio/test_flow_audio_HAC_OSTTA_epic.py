...
from dataloader_flow_audio_HAC_OSTTA_epic import HACDOMAIN
...
def validate_one_step(flow, labels, spectrogram, flow_open, labels_open, spectrogram_open):
    if args.use_flow:
    ...
    if args.online_adapt:
        ...
        l1_distance = torch.sum(torch.abs(nn.Softmax(dim=1)(f_predict) - nn.Softmax(dim=1)(audio_predict)), dim=1)
        loss_a2d_dis = (-l1_distance * tanh_w).mean()

        pred_ent_v = normalized_prediction_entropy(f_predict)
    ...
    return predict, f_predict, audio_predict
...
if __name__ == '__main__':
    ...
    test_dataset = HACDOMAIN(split='test', source=False, domain=args.target_domain, cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath, datapath_open=args.datapath_open, use_flow=args.use_flow, use_audio=args.use_audio)
    ...
    for epoch_i in range(args.nepochs):
        for split in ['test']:
            ...
            with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                for (i, (flow, spectrogram, labels, flow_open, spectrogram_open, labels_open)) in enumerate(dataloaders[split]):
                    
                    for _ in range(args.steps):
                        predict1, f_predict, audio_predict = validate_one_step(flow, labels, spectrogram, flow_open, labels_open, spectrogram_open)
