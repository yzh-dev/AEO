...
def validate_one_step(clip, labels, spectrogram, clip_open, labels_open, spectrogram_open):
    ...
    if args.online_adapt:
        ...
        # l1_distance_f = torch.sum(torch.abs(nn.Softmax(dim=1)(predict) - nn.Softmax(dim=1)(audio_predict)), dim=1)
        # l1_distance_v = torch.sum(torch.abs(nn.Softmax(dim=1)(predict) - nn.Softmax(dim=1)(predict1)), dim=1)
        # loss_a2d_dis = ((-l1_distance * tanh_w).mean() + (-l1_distance_f * tanh_w).mean() + (-l1_distance_v * tanh_w).mean())/3
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t2','--target_domain2', nargs='+', help='<Required> Set target_domain2', required=True)
    ...
    parser.add_argument('--a2d_ratio', type=float, default=1.0,
                        help='a2d_ratio')
    ...
    parser.add_argument("--max_iter", type=int, default=440)
    ...
    test_dataset2 = HACDOMAIN(split='test', source=False, domain=args.target_domain2, cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath, datapath_open=args.datapath_open, use_video=args.use_video, use_audio=args.use_audio)
    test_dataloader2 = torch.utils.data.DataLoader(test_dataset2, batch_size=args.bsz, num_workers=args.num_workers, shuffle=True,
                                                   pin_memory=(device.type == "cuda"), drop_last=False)
    dataloaders = {'test': test_dataloader, 'test2': test_dataloader2}
