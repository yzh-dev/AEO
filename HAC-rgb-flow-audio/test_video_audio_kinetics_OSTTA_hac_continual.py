...
from dataloader_video_audio_kinetics_OSTTA_hac import HACDOMAIN
...
def validate_one_step(clip, labels, spectrogram, clip_open, labels_open, spectrogram_open):
    ...
    # print("clip.shape: ", clip.shape) # torch.Size([64, 3, 32, 224, 224])
    # print("spectrogram.shape: ", spectrogram.shape) # torch.Size([64, 1, 257, 1004])
    ...
    if args.online_adapt:
        ...
        # loss = loss_ra + args.a2d_ratio * loss_a2d_dis - args.marginal_ent_wei * loss_bal
...
if __name__ == '__main__':
    ...
    parser.add_argument('--marginal_ent_wei', type=float, default=1.0,
                        help='marginal_ent_wei')
    parser.add_argument('--selective_ent_min_with_a2d', action='store_true')
    parser.add_argument('--a2d_ratio', type=float, default=0.05,
                        help='a2d_ratio')
    parser.add_argument('--tanh_k', type=float, default=4.0,
                        help='tanh_k')
    parser.add_argument('--tanh_alpha', type=float, default=0.9,
                        help='tanh_alpha')

    parser.add_argument("--dataset", type=str, default='Kinetics') 
    parser.add_argument("--audio_noise_type", type=str, default='gaussian_noise') 
    # wind            traffic     thunder       rain       crowd              gaussian_noise
    parser.add_argument("--video_noise_type", type=str, default='gaussian_noise') 
    # defocus_blur    frost       brightness    pixelate   jpeg_compression   gaussian_noise
    ...
    if args.use_kinetics_100:
        log_name = "log_video_audio_Kinetics_100"
    else:
        log_name = "log_video_audio_%s"%(str(args.dataset))
    ...
    if args.online_adapt:
        ...
        if args.use_scheduler:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, args.max_iter)

    video_noise_types = ["defocus_blur", "frost", "brightness", "pixelate", "jpeg_compression", "gaussian_noise_5"]
    audio_noise_types = ["wind", "traffic", "thunder", "rain", "crowd", "gaussian_noise"]

    for noise_type_i in range(len(video_noise_types)):
        test_dataset = HACDOMAIN(split='test', cfg=cfg, cfg_flow=cfg_flow, use_kinetics_100=args.use_kinetics_100, audio_noise_type=audio_noise_types[noise_type_i], video_noise_type=video_noise_types[noise_type_i], datapath=args.datapath, datapath_open=args.datapath_open, use_video=args.use_video, use_flow=args.use_flow, use_audio=args.use_audio)
        ...
        for epoch_i in range(args.nepochs):
            for split in ['test']:
                ...
                with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                    ...
                    accuracy = acc(pred, label_list, ood_label=num_class)
                    #print("log_name_np: ", log_name_np)
                    print(video_noise_types[noise_type_i], audio_noise_types[noise_type_i])
                    print("acc: ", accuracy)

                    label_list[label_list==num_class] = -1
