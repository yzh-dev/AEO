...
import random
from dataloader_video_flow_HAC_OSTTA_epic import HACDOMAIN
...
def validate_one_step(clip, labels, flow, clip_open, labels_open, flow_open):
    if args.use_video:
    ...
    if args.online_adapt:
        ...
        l1_distance = torch.sum(torch.abs(nn.Softmax(dim=1)(predict1) - nn.Softmax(dim=1)(f_predict)), dim=1)
    ...
    return predict, predict1, f_predict
...
if __name__ == '__main__':
    ...
    test_dataset = HACDOMAIN(split='test', source=False, domain=args.target_domain, cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath, datapath_open=args.datapath_open, use_video=args.use_video, use_flow=args.use_flow)
            ...
            with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                for (i, (clip, flow, labels, clip_open, flow_open, labels_open)) in enumerate(dataloaders[split]):
                    ...
                    for _ in range(args.steps):
                        predict1, v_predict, f_predict = validate_one_step(clip, labels, flow, clip_open, labels_open, flow_open)
                    
                    _, predict = torch.max(predict1.detach().cpu(), dim=1)
                    count = int((predict1.size()[0])/2)
                    acc1 = (predict[:count] == labels).sum().item()
