...
from dataloader_video_flow_audio_HAC_OSTTA_epic import HACDOMAIN
...
def validate_one_step(clip, labels, flow, spectrogram, clip_open, labels_open, flow_open, spectrogram_open):
    if args.use_video:
    ...
    if args.use_flow:
        ...
        flow_open = flow_open['imgs'].cuda().squeeze(1)
        flow = torch.cat((flow, flow_open), dim=0)
    ...
    if args.online_adapt:
        ...
        l1_distance2 = torch.sum(torch.abs(nn.Softmax(dim=1)(f_predict) - nn.Softmax(dim=1)(audio_predict)), dim=1)
        l1_distance3 = torch.sum(torch.abs(nn.Softmax(dim=1)(predict1) - nn.Softmax(dim=1)(f_predict)), dim=1)
        loss_a2d_dis = ((-l1_distance * tanh_w).mean() + (-l1_distance2 * tanh_w).mean() + (-l1_distance3 * tanh_w).mean())/3

        pred_ent_v = normalized_prediction_entropy(predict1)
        pred_ent_a = normalized_prediction_entropy(audio_predict)
        pred_ent_f = normalized_prediction_entropy(f_predict)
        loss_a2d_ent = ((-pred_ent_v * tanh_w).mean() + (-pred_ent_a * tanh_w).mean() + (-pred_ent_f * tanh_w).mean())/3
    ...
    return predict, predict1, f_predict, audio_predict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tanh_k', type=float, default=4.0,
                        help='tanh_k')
    parser.add_argument('--tanh_alpha', type=float, default=0.7,
                        help='tanh_alpha')
    ...
    test_dataset = HACDOMAIN(split='test', source=False, domain=args.target_domain, cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath, datapath_open=args.datapath_open, use_video=args.use_video, use_flow=args.use_flow, use_audio=args.use_audio)
    ...
    for epoch_i in range(args.nepochs):
        for split in ['test']:
            ...
            if args.use_flow:
                model_flow.train(args.online_adapt)

            label_list, output_list = [], []

            with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                for (i, (clip, flow, spectrogram, labels, clip_open, flow_open, spectrogram_open, labels_open)) in enumerate(dataloaders[split]):
                    
                    for _ in range(args.steps):
                        predict1, v_predict, f_predict, audio_predict = validate_one_step(clip, labels, flow, spectrogram, clip_open, labels_open, flow_open, spectrogram_open)
